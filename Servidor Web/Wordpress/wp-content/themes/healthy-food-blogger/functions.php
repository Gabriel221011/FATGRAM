...
add_action('init', 'cpt_restaurantes');
...
add_action( 'grunion_pre_message_sent', 'crear_restaurante_desde_jetpack', 10, 3 );

// Enviar los datos del formulario a /resturantes
function procesar_formulario_restaurante() {
    // 1. Verificar si se ha enviado el formulario
    if ( isset($_POST['enviar_restaurante']) && isset($_POST['res_nonce']) ) {
        
        // 2. Validar seguridad (Nonce)
        if ( ! wp_verify_nonce($_POST['res_nonce'], 'crear_restaurante_accion') ) {
            return;
        }

        // 3. Recoger y limpiar datos
        $nombre      = sanitize_text_field($_POST['res_nombre']);
        $descripcion = sanitize_textarea_field($_POST['res_descripcion']);
        $zona        = sanitize_text_field($_POST['res_zona']);
        $valoracion  = sanitize_text_field($_POST['res_valoracion']);

        // 4. Crear el array de la entrada
        $nuevo_restaurante = array(
            'post_title'   => $nombre,
            'post_content' => $descripcion,
            'post_status'  => 'publish', // O 'pending' si quieres revisarlos antes
            'post_type'    => 'restaurante',
        );

        // 5. Insertar en la base de datos
        $post_id = wp_insert_post($nuevo_restaurante);

        // 6. Guardar los campos personalizados (ACF o Meta)
        if ( ! is_wp_error($post_id) ) {
            // Guardamos la zona y valoración en los metadatos
            update_post_meta($post_id, 'zona_restaurante', $zona);
            update_post_meta($post_id, 'valoracion', $valoracion);

            // Redirigir para evitar reenvío de formulario
            wp_redirect( home_url('/restaurantes/?creado=exito') );
            exit;
        }
    }
}
add_action('template_redirect', 'procesar_formulario_restaurante');
