        ...
        <div class="nav-icons">
            ...
            <?php  
                if ( is_user_logged_in() ) :  
                    ...
                    $notificaciones = obtener_notificaciones_usuario($user->ID);
                    $no_leidas = 0;

                    if ($notificaciones) {
                        foreach ($notificaciones as $n) {
                            if (!get_post_meta($n->ID, 'leida', true)) {
                                $no_leidas++;
                            }
                        }
                    }
                ?>
                    <a href="<?php echo home_url('/notificaciones/'); ?>" class="notif-icon">
                        <i class="fas fa-bell"></i>
                        <?php if ($no_leidas > 0) : ?>
                            <span class="notif-count"><?php echo $no_leidas; ?></span>
                        <?php endif; ?>
                    </a>
                <?php endif; ?>
