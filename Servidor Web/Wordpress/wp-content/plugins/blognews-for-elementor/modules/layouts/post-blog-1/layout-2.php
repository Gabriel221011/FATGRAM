<?php
?>
<div id="post-<?php the_ID(); ?>" <?php post_class('blogfoel-post-item '. $image_class); ?>>
            ...
            foreach ($active_repeater as $post_item) :
                ...
                if ($content_type === 'meta' && !empty($post_meta_repeater)) {
                    ...
                    foreach ($post_meta_repeater as $meta_item) {
                        ...
                        $date_format          = 'default';
                        // print_r($icon);
                        // error_log(print_r($icon, true));
                        if ($meta_type === 'author') {
                            $meta_html .= blogfoel_get_author($avatar, $icon);
                        } elseif ($meta_type === 'date') {
                            $meta_html .= blogfoel_get_date($date_format, $icon);
                        } elseif ($meta_type === 'comments') {
                            $meta_html .= blogfoel_get_comments($icon, $no_comments, $one_comment, $multi_comments);
